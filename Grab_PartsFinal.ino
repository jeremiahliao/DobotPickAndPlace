...
//      delay(500);
      SetEndEffectorGripper(true, false, true, &gQueuedCmdIndex); // open gripper (compressed air on);
      ProtocolProcess(); //have command(s) executed by dobot
      delay(500);
      SetEndEffectorGripper(false, false, true, &gQueuedCmdIndex);  // stop activating gripper when it is openend (compressed air off)
      delay(500); 
      ProtocolProcess();
      }
  else if(vacuumOn == true && vacuumOn != currentVac) {
    Serial.println("Close GRIPPER");
    SetEndEffectorGripper(true, true, true, &gQueuedCmdIndex);
    delay(500);
    ProtocolProcess();
  }
}
else{
if (vacuumOn == false) SetEndEffectorSuctionCup(false, true, true, &gQueuedCmdIndex);
if (vacuumOn == true && vacuumOn != currentVac)SetEndEffectorSuctionCup(true, true, true, &gQueuedCmdIndex);
ProtocolProcess();
}
...
moveArm(dobotPartX, dobotPartY, (zDownPos+40), currentR, currentVac);
delay(2000);
